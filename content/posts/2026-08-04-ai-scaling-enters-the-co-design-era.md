...
同じGPUを使っても、どのトークンを参照し、どのexpertを動かし、何bitで重みを持ち、どの順序で通信するかによって、実際に動かせるモデルの規模とコンテキスト長が変わる。
...
[Kimi K3の技術報告](https://arxiv.org/abs/2607.24653)は、モデルを系列長、深さ、幅という三方向から組み替えている。
...
Kimi K3は、三つの方向すべてで情報の流れを選択的にしている。
...
Kimi K3が狙っているのは、小さいモデルで済ませることではない。
巨大なモデルを、長いコンテキストでも動かせるようにすることである。

## DeepSeek V4はモデルからGPUまで変える

Kimi K3が三方向の情報の流れを選別する一方で、DeepSeek V4はモデルの接続方法からGPU上の実装までを一体で変えている。
...
Attention、MoE、residual、optimizer、数値表現、kernel、通信を、一つの実行可能なシステムとして成立させる研究が中心へ戻ってきた。
